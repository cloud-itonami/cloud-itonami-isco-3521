(ns media.actor
  "MediaBroadcastActor — the ISCO-08 3521 community media actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one media operation request
  ...
  The unconditional invariant: the MediaBroadcastAdvisor can never
  directly commit a record the MediaBroadcastGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [media.advisor :as advisor]
            [media.governor :as governor]
            [media.store :as store]))

(defn build-graph
  "Build a compiled MediaBroadcastActor graph. `store` implements
  `media.store/Store`. `advisor` implements
  `media.advisor/Advisor` (defaults to `mock-advisor`).
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:client-id (:client-id request)
                                    :op (:op proposal)
                                    :article-id (:article-id proposal)
                                    :payload proposal}]
